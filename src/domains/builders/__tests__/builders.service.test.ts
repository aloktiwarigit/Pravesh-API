...
describe('BuildersService', () => {
  ...
  beforeEach(() => {
    ...
    service = new BuildersService(mockPrisma);
    vi.clearAllMocks();
  });
  ...
  describe('registerBuilder', () => {
    it('should create builder with PENDING_VERIFICATION status', async () => {
      (mockPrisma.builder.findUnique as any).mockResolvedValue(null);
      (mockPrisma.builder.create as any).mockResolvedValue({
        id: 'builder-1',
    ...
    it('should reject duplicate RERA numbers', async () => {
      (mockPrisma.builder.findUnique as any).mockResolvedValue({
        id: 'existing',
        reraNumber: 'UPRERAPRJ123456',
      });
  ...
  describe('approveBuilder', () => {
    it('should set VERIFIED status and verifiedAt on approval', async () => {
      ...
      (mockPrisma.builder.update as any).mockResolvedValue({
        id: 'builder-1',
        status: 'VERIFIED',
        verifiedAt: new Date(),
      });
    ...
    it('should reject approval if builder not in PENDING_VERIFICATION', async () => {
      ...
      await expect(service.approveBuilder('builder-1', 'approve')).rejects.toThrow(
  ...
  describe('createProject', () => {
    ...
    it('should create project with ACTIVE status', async () => {
      ...
      (mockPrisma.builderProject.create as any).mockResolvedValue({
        id: 'project-1',
        status: 'ACTIVE',
        totalUnits: 48,
      });
  ...
  describe('createBroadcast', () => {
    it('should enforce 2 broadcasts per week limit', async () => {
      (mockPrisma.builderBroadcast.count as any).mockResolvedValue(2);
