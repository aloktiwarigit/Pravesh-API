...
export async function registerWireTransferSlaJob(
  boss: any,
  service: WireTransferService
) {
  await boss.createQueue('wire-transfer.sla-check');

  // Schedule daily at 9 AM IST (3:30 AM UTC)
  await boss.schedule('wire-transfer.sla-check', '30 3 * * *');

  await boss.work('wire-transfer.sla-check', async () => {
